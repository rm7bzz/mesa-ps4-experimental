...
void
ac_build_sampler_descriptor(const struct radeon_info *info, const struct ac_sampler_state *state,
                            uint32_t desc[4])
{
   ...
   /*
    * GNM exposes ANISO_THRESHOLD and ANISO_BIAS as independent fields; its
    * sampler constructor leaves both zero instead of deriving them here.
    */
   desc[0] = S_008F30_CLAMP_X(state->address_mode_u) |
             ...
             S_008F30_ANISO_THRESHOLD(is_ps4 ? 0 : state->max_aniso_ratio >> 1) |
             S_008F30_ANISO_BIAS(is_ps4 ? 0 : state->max_aniso_ratio) |
   ...
   desc[1] = 0;
   /* Vulkan/Gallium minification applies to Z as well; GNM defaults it to POINT. */
   desc[2] = S_008F38_XY_MAG_FILTER(state->mag_filter) |
             S_008F38_XY_MIN_FILTER(state->min_filter) |
             S_008F38_Z_FILTER(is_ps4 ? 1 + (state->min_filter & 1) : 0) |
   ...
   if (gfx_level >= GFX12) {
      ...
      desc[2] |= S_008F38_PERF_MIP_LO(state->perf_mip);
      desc[3] |= S_008F3C_PERF_MIP_HI(state->perf_mip >> 2);
   } else {
      desc[1] |= S_008F34_MIN_LOD_GFX6(util_unsigned_fixed(CLAMP(state->min_lod, 0, 15), 8)) |
                 ...
                 S_008F34_PERF_MIP(state->perf_mip);
   }
   ...
   if (gfx_level >= GFX10) {
      ...
      desc[2] |= S_008F38_LOD_BIAS(util_signed_fixed(CLAMP(state->lod_bias, -16, 16), 8)) |
                 S_008F38_DISABLE_LSB_CEIL(gfx_level <= GFX8 && !is_ps4) |
                 S_008F38_FILTER_PREC_FIX(!is_ps4) |
