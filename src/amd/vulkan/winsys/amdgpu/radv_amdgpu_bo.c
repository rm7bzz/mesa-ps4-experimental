...
radv_amdgpu_virtual_bo_clear_mapping(struct radv_amdgpu_winsys *ws, struct radv_amdgpu_winsys_bo *bo)
{
   ...
   /* CLEAR with bo_handle=0 removes ALL entries from the kernel's VM interval
    * tree for this VA range.  This works regardless of PRT support and correctly
    * handles stale entries from backing BOs that were freed while still bound
    * (ranges with bo=NULL that per-range UNMAP would skip).
    *
    * The kernel allows handle=0 for CLEAR and operates on the VM directly.
    */
                           ...
                           uint64_t size, struct radv_amdgpu_winsys_bo *bo, uint64_t bo_offset)
{
   ...
   /* REPLACE atomically removes any overlapping entries from the kernel's
    * VM interval tree and then adds the new mapping.  This works even if
    * there is no prior mapping (the clear step is a no-op) and correctly
    * handles partial overlaps with previous MAP operations.  It does NOT
    * require PRT support.
    */
...
radv_amdgpu_winsys_bo_destroy(struct radeon_winsys *_ws, struct radeon_winsys_bo *_bo)
{
   ...
   const uint64_t va_size = radv_amdgpu_bo_va_size(bo->base.size, bo->flags);
   /* UNMAP the main BO mapping. */
   radv_amdgpu_bo_va_op(ws, bo->bo_handle, 0, bo->base.size, bo->base.va, 0, 0, AMDGPU_VA_OP_UNMAP);
   /* The kernel's UNMAP only matches by start address (ignores map_size), so the
    * padding page that was MAPped separately at a different VA must be UNMAPped
    * with its own call.  Without this, every freed BO with VM_PAD_1PAGE leaks a
    * stale 1-page entry in the kernel's VM interval tree.
    */
   if (bo->flags & RADEON_FLAG_VM_PAD_1PAGE) {
      const uint64_t pad_va = bo->base.va + align64(bo->base.size, 4096);
      radv_amdgpu_bo_va_op(ws, bo->bo_handle, 0, 4096, pad_va, 0, 0, AMDGPU_VA_OP_UNMAP);
   }
                             ...
                             uint64_t replay_address, struct radeon_winsys_bo **out_bo)
{
   ...
   *out_bo = NULL;
   ...
   r = ac_drm_va_range_alloc(ws->dev, amdgpu_gpu_va_range_general, va_size + va_gap_size, virt_alignment, replay_address,
                             &va, &va_handle, va_flags);
   if (r) {
      fprintf(stderr, "radv/amdgpu: VA range alloc failed: size=%" PRIu64 " align=%u err=%d\n",
              va_size + va_gap_size, virt_alignment, r);
      result = replay_address ? VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS : VK_ERROR_OUT_OF_DEVICE_MEMORY;
      goto error_va_alloc;
   }
   ...
   /* On APUs with small GTT (e.g. PS4 Liverpool), HOST_VISIBLE allocations
    * larger than GTT fail because the kernel routes CPU_ACCESS_REQUIRED
   ...
   if (r) {
      fprintf(stderr, "radv/amdgpu: VA MAP failed for bo: size=%" PRIu64 " VA=0x%" PRIx64 " err=%d\n",
              size, va, r);
      result = VK_ERROR_OUT_OF_DEVICE_MEMORY;
      goto error_va_map;
   }
   ...
   if (flags & RADEON_FLAG_VM_PAD_1PAGE) {
      ...
      if (r) {
         fprintf(stderr, "radv/amdgpu: VA MAP failed for pad page: VA=0x%" PRIx64 " err=%d\n", pad_va, r);
         result = VK_ERROR_OUT_OF_DEVICE_MEMORY;
         goto error_va_map;
      }
   }
