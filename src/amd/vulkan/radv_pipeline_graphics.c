                             ...
                             bool blend_need_alpha)
{
   ...
   /* GFX7 workaround: SPI_SHADER_UNORM16_ABGR and SPI_SHADER_SNORM16_ABGR
   ...
   if (pdev->info.gfx_level <= GFX7 &&
      ...
      formats.normal = formats.blend;
      formats.alpha = formats.blend_alpha;
   }
                                 ...
                                 VkGraphicsPipelineLibraryFlagBitsEXT lib_flags, uint32_t custom_blend_mode)
{
   ...
   /* GFX7 and below bounds-check the MUBUF offset field against the stride, so any
    * attribute byte offset >= stride will read zeros.  Force the VS prolog path which
    * embeds the attribute offset into the buffer descriptor base address instead. */
   if (pdev->info.gfx_level <= GFX7) {
      key.vs.has_prolog = true;
   }
